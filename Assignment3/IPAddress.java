...
public class IPAddress implements Comparable<IPAddress>{
	...
	public int hashCode() {
		int hash = 1;
		hash = hash * 3 + network;
		hash = hash * subnet2 + subnet;
		hash = hash *5 + subnet2/2;
		hash = hash *6 - host;
		return hash;
	}
	/**
	...
	/**
	 * Returns a positive integer if it is greater.
	 * otherwise returns a negative.
	 * @return a integer associated to the compareTo.
	 */
	@Override
	public int compareTo(IPAddress obj) {
		if(((Integer) this.network).compareTo(obj.network)!=0)
			return ((Integer)this.network).compareTo(obj.network);
		if(((Integer) this.subnet).compareTo(obj.subnet)!=0)
			return ((Integer)this.subnet).compareTo(obj.subnet);
		if(((Integer) this.subnet2).compareTo(obj.subnet2)!=0)
			return ((Integer)this.subnet2).compareTo(obj.subnet2);
		return ((Integer)this.host).compareTo(obj.host);
	}
}
